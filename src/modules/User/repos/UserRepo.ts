 ...
 * @returns {Promise<IUser>} A promise that resolves to the stored or updated user.
 * @throws {Error} Throws an error if storing or updating fails.
 */
async function storeUserDB(user: IUser): Promise<IUser> {
  try {
    const text = `INSERT INTO users(name, type, location, bio, publicRepos, followers, following, createdAt, updatedAt)
...
async function getUsersDB(params: IUserSearchParams): Promise<IUser[]> {
  try {
    ...
    if (params.language) {
      text +=
        ' INNER JOIN UserLanguages ON Users.id = UserLanguages.userId AND LOWER(language) LIKE LOWER($1)'
